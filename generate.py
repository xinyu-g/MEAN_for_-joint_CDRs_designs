...
def set_cdr(cplx, seq, x, cdr_list=['H1', 'H2', 'H3']):
...
def eval_one(tup, out_dir, cdr_list=['H1', 'H2', 'H3']):
    ...
    # Set CDRs
    new_cplx = set_cdr(cplx, seq, x, cdr_list)
    pdb_path = os.path.join(out_dir, cplx.get_id() + '.pdb')
    new_cplx.to_pdb(pdb_path)
    print(f'save to {pdb_path}')
    summary['TMscore'] = tm_score(cplx.get_heavy_chain(), new_cplx.get_heavy_chain())

    # AAR (Amino Acid Recovery) for each CDR in the list
    total_hits = 0
    total_length = 0
    for cdr in cdr_list:
        origin_seq = cplx.get_cdr(cdr).get_seq()
        hits = sum(1 for a, b in zip(origin_seq, seq) if a == b)
        total_hits += hits
        total_length += len(origin_seq)
    summary['AAR'] = total_hits / total_length if total_length > 0 else 0
    return summary
...
def rabd_test(args, model, test_set, test_loader, device):
    ...
    # print('global_best_ppl:', global_best_ppl)
    ...
    with torch.no_grad():
        for _ in tqdm(range(args.rabd_sample)):
            ...
            # print('ppl:', len(ppl), len(test_set))
    ...
    for k in range(args.topk):
        ...
        summaries = process_map(partial(eval_one, out_dir=k_out_dir, cdr_list=cdr_list), inputs, max_workers=args.num_workers, chunksize=10)
...
def average_test(args, model, test_set, test_loader, device):
    ...
    for _round in range(args.run):
        ...
        print(len(test_set), len(results))
        ...
        cdr_list = ['H1', 'H2', 'H3']
        summaries = process_map(partial(eval_one, out_dir=out_dir, cdr_list=cdr_list), inputs, max_workers=args.num_workers, chunksize=10)
